let clear_names = {
    ...
    "tm0":["T-Spin Mini","rgb(189, 0, 189)"],
    "tm1":["T-Spin Mini Single","rgb(189, 0, 189)"],
    "tm2":["T-Spin Mini Double","rgb(189, 0, 189)"]
}
...
if (getCookie("controls") && JSON.parse(getCookie("controls")).version==3) {
...
function key_selector(control, controls) {
    let input = document.createElement("input")
    let text = document.createTextNode(texts.controls[control][lang] + ":")
...
function number_selector(control, controls) {
    let input = document.createElement("input")
    let text = document.createTextNode(texts.controls[control][lang] + ":")
    input.value = controls[control]
...
function radio_selector(control, labels, labels_to_values, controls) {
    inputs = document.createElement("div")
    for (let label of labels){
        let input = document.createElement("input")
        input.type="radio"
        input.name=control
        input.id=`${control}_radio_${label}`
        input.value=labels_to_values[label]
        if (controls[control]==input.value) {input.checked = true}
        let label_node=document.createElement("label")
        label_node.for=`${control}_radio_${label}`
        label_node.textContent=label
        input.onclick=()=>controls[control]=input.value
        appendChildren(inputs,[input,label_node])
    }
    let text = document.createTextNode(texts.controls[control][lang] + ":")
    return [text, inputs]
}
...
function opacity_selector(controls) {
    let input = document.createElement("input")
    input.type = "range"
    let text = document.createTextNode(texts.controls.opacity[lang]+":")
    input.min=0
    input.max=1
    input.step=0.05
...
document.querySelector("#control_button").src=images_lang.settings[lang]
{
    appendChildren(controls_select_grid, key_selector("hd", controls))
    appendChildren(controls_select_grid, key_selector("sd", controls))
    appendChildren(controls_select_grid, key_selector("left", controls))
    appendChildren(controls_select_grid, key_selector("right", controls))
    appendChildren(controls_select_grid, key_selector("cw", controls))
    appendChildren(controls_select_grid, key_selector("ccw", controls))
    appendChildren(controls_select_grid, key_selector("double_rotate", controls))
    appendChildren(controls_select_grid, key_selector("hold",  controls))
    appendChildren(controls_select_grid, key_selector("reset", controls))
    appendChildren(controls_select_grid, number_selector("das", controls))
    appendChildren(controls_select_grid, number_selector("arr", controls))
    appendChildren(controls_select_grid, number_selector("sd_arr", controls))
    appendChildren(controls_select_grid, opacity_selector(controls))
    appendChildren(controls_select_grid, radio_selector("lang",["Français","English"],{"Français":"fr","English":"en"}, controls))
    addCommands(controls)
    document.querySelector("#controls_back").textContent=texts.controls.back[lang]
}   
...
function end_select_controls() {
    ...
    if (controls.lang != lang) location.reload() 
}
...
function win_screen(score) {
    ...
    if (getCookie(win_condition.cookie)) {
        ...
        if (win_condition.is_better(score, oldScore)) {
            ...
            pb_label.textContent = texts.win_screen.new_best[lang] + win_condition.format_score(oldScore)
        }
        else {
            pb_label.textContent = texts.win_screen.pb[lang] + win_condition.format_score(oldScore)
        }
    }
    else {
        setCookie(win_condition.cookie, score)
        pb_label.textContent =texts.win_screen.first_game[lang]
    }
    document.removeEventListeners()
    let score_node = document.createElement("pre")
    win_box.appendChild(score_node)
    score_node.textContent = `${win_condition.score_label}: ${win_condition.format_score(score)}\n${pps} PPS\n${grid.placed_pieces} ${texts.win_screen.pieces[lang]}`
    let close_button = document.createElement("button")
    close_button.textContent = texts.win_screen.back[lang]
...
function reset() {
    ...
    if (pb) pb_node.textContent = `${texts.ingame_text.best[lang]} ${win_condition.score_label}\n` + win_condition.format_score(pb)
    else pb_node.textContent=''
    grid.reset()
    start_time = undefined
    piece_spawn()
    document.removeEventListeners()
...
let sprint_win_condition = {
    ...
    cookie: "highScore",
    score_label:texts.gamemodes.sprint.score_label[lang],
    is_better: (score1, score2)=> score1<score2,
    format_score: (score)=>displayable_time(score, 2),
    mission: texts.gamemodes.sprint.mission[lang],
    lines_label: "/40"
}
let ultra_win_condition = {
    ...
    score_label:texts.gamemodes.ultra.score_label[lang],
    is_better: (score1, score2)=> score1>score2,
    format_score: score=>score,
    mission: texts.gamemodes.ultra.mission[lang],
    lines_label: ""
}
...
clear_node.parentNode.style.top = 220 * height_factor + "px"
